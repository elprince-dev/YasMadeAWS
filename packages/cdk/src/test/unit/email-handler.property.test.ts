...
const emailArb: fc.Arbitrary<string> = fc
  .tuple(
    fc.array(fc.constantFrom(...alphaNum.split('')), { minLength: 1, maxLength: 8 }),
    fc.array(fc.constantFrom(...alpha.split('')), { minLength: 1, maxLength: 6 }),
...
const validOrderConfirmationArb: fc.Arbitrary<OrderConfirmationRequest> = fc.record({
  ...
  subtotal: fc.float({ min: Math.fround(0), max: Math.fround(9999), noNaN: true }),
  shippingFee: fc.float({ min: Math.fround(0), max: Math.fround(999), noNaN: true }),
  discountAmount: fc.float({ min: Math.fround(0), max: Math.fround(999), noNaN: true }),
  totalAmount: fc.float({ min: Math.fround(0), max: Math.fround(9999), noNaN: true }),
  paymentProofUrl: nonEmptyString.map((s: string) => `https://example.com/${s}`),
})

// Helper: remove a random required key from an object
function removeRandomKey(obj: Record<string, unknown>, keys: string[]): Record<string, unknown> {
  const copy = { ...obj }
...
describe('Property 1: Request validation rejects incomplete payloads', () => {
  const NEWSLETTER_REQUIRED_KEYS = ['subscribers', 'subject', 'content', 'logoUrl']
  const CONTACT_REQUIRED_KEYS = ['name', 'email', 'subject', 'message']
  const ORDER_REQUIRED_KEYS = [
    'orderId', 'customerEmail', 'customerName', 'items',
    'shippingAddress', 'subtotal', 'shippingFee', 'discountAmount',
    'totalAmount', 'paymentProofUrl',
  ]
...
// We test this by importing the handler and mocking SES at the module level
// Since the handler uses SES internally, we mock @aws-sdk/client-ses
jest.mock('@aws-sdk/client-ses', () => {
  const sentEmails: string[][] = []
  return {
    SESClient: jest.fn().mockImplementation(() => ({
      send: jest.fn().mockImplementation((command: unknown) => {
        const cmd = command as { input: { Destination: { ToAddresses: string[] } } }
...
import { handler } from '../../domains/email/lambda/email-handler'
import type { APIGatewayProxyEventV2 } from 'aws-lambda'

// eslint-disable-next-line @typescript-eslint/no-require-imports
const sesMock = require('@aws-sdk/client-ses') as {
  __sentEmails: string[][]
  __clearSentEmails: () => void
}

function makeEvent(path: string, body: unknown, authHeader?: string): APIGatewayProxyEventV2 {
  return {
    rawPath: path,
  ...
  test('for any valid newsletter request, the handler attempts to send to every subscriber', async () => {
    await fc.assert(
      fc.asyncProperty(validNewsletterArb, async (req) => {
        ...
        const responseBody = JSON.parse(typeof result === 'string' ? result : (result as { body: string }).body)

        expect(responseBody.success).toBe(true)

        // Every subscriber should have been sent to
        const allRecipients = sesMock.__sentEmails.flat()
...
describe('Property 4: Newsletter HTML template contains required elements', () => {
  test('for any content and logo URL, rendered HTML contains logo, content, signature, and footer', () => {
    fc.assert(
      fc.property(nonEmptyString, nonEmptyString, nonEmptyString.map((s: string) => `https://example.com/${s}`), (subject, content, logoUrl) => {
...
describe('Property 5: Plain text generation strips all HTML', () => {
  const htmlTagArb = fc.constantFrom(
    'div', 'p', 'span', 'a', 'img', 'br', 'h1', 'h2', 'h3',
    'table', 'tr', 'td', 'strong', 'em', 'ul', 'li', 'hr',
  )
  ...
  const htmlContentArb = fc
    .array(
      fc.oneof(
        nonEmptyString,
        fc.tuple(htmlTagArb, nonEmptyString).map(([tag, text]) => `<${tag}>${text}</${tag}>`),
...
describe('Property 6: Image URL absolutization', () => {
  ...
  test('for any HTML with relative img src, absolutizeImageUrls makes all src absolute', () => {
    ...
    fc.assert(
      fc.property(htmlWithRelativeImagesArb, (html) => {
        ...
        const srcMatches = [...result.matchAll(/<img[^>]*\ssrc="([^"]+)"[^>]*>/gi)]
  ...
  test('for any HTML with absolute img src, absolutizeImageUrls leaves them unchanged', () => {
    const absoluteUrlArb = nonEmptyString.map((s: string) => `https://cdn.example.com/${s}`)
  ...
  test('for any valid contact request, handler sends to admin with reply-to set to visitor email', async () => {
    await fc.assert(
      fc.asyncProperty(validContactArb, async (req) => {
        ...
        expect(sesMock.__sentEmails.length).toBeGreaterThan(0)
      }),
      { numRuns: 100 },
    )
  })
  ...
  test('for any valid order confirmation request, handler sends to both customer and admin', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(validOrderConfirmationArb, async (req) => {
        ...
        const event = makeEvent('/order-confirmation', req, 'Bearer valid-token')
        const result = await handler(event)
        const responseBody = JSON.parse(typeof result === 'string' ? result : (result as { body: string }).body)
